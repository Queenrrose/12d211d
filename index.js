...
const MONITORING_SERVERS = [
  ...
  {
    guildId: '',  // معرف السيرفر الثاني
    channels: [''], // القناة الوحيدة في السيرفر الثاني
  },
...
const clients = BOT_TOKENS.map((token, index) => {
  const client = new ClientStructure({
    ...
    intents: [
      ...
      Intents.FLAGS.GUILD_VOICE_STATES,  // إضافة هذا السطر لمتابعة تغيرات الحالة في القنوات الصوتية
    ],
  });
